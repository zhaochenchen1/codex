---
name: hithink-management-query
description: 查询股本结构、股权结构、股东户数、前十大股东/流通股东、主要持有人、实控人等股权信息，支持自然语言问句输入，返回相关股东股本数据结果。当用户询问股本结构、股东户数、前十大股东、股权质押、实控人、主要持有人等股东股本数据查询问题时，必须使用此技能。
...
- 股权结构（国有股、法人股、社会公众股等）
- 股东户数（股东总户数、户均持股等）
- 前十大股东/流通股东（持股比例、持股数量等）
- 主要持有人（机构持仓、个人持仓等）
- 实控人信息（实际控制人、控股比例等）
- 股权质押情况
- 高管信息（董事会成员、监事会成员、高管团队等）
- 支持自然语言问句输入，返回相关股东股本数据结果
...
接收用户的自然语言查询请求，分析用户意图，识别股东股本相关查询类型：
- 股本结构/股权结构查询
- 股东户数/前十大股东查询
- 实控人/高管信息查询
...
| `X-Claw-Skill-Id` | `hithink-management-query`（与 skill name 一致） |
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含股票代码、股票简称、股东名称、持股比例等字段 |
...
# 查询前十大股东
python3 scripts/cli.py --query "前十大股东"

# 查询实控人信息
python3 scripts/cli.py --query "实控人信息"

# 翻页查询
python3 scripts/cli.py --query "股东信息" --page 2 --limit 20

# 重试请求（放宽条件后使用 retry 标记）
python3 scripts/cli.py --query "同花顺股本结构" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "同花顺股本结构" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-management-query" \
...
} -Body '{"query":"同花顺股本结构","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
